...
function revrot(str, sz) {
  if(sz <= 0 || str.length === '' || str.length < sz) return ''

  let arr = str.split('')
  let result = ''

  for(let i = 0; i < Math.floor(str.length / sz); i++){
    let spliceStr = arr.splice(0, sz)
    let sum = spliceStr.reduce((acc, current) => acc+= +current, 0)

    if(sum % 2 === 0){
      result += spliceStr.reverse().join('')
    }else{
      result += spliceStr.slice(1).join('') + spliceStr[0]
    }
  }
  return result
}
